...
class PostActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val takePicRes =  registerForActivityResult(ActivityResultContracts.TakePicture()) { success ->
            if (success) {
                doCrop(cropIntent)
            } else {
        ...
        view.cameraButton.setOnClickListener {
            viewState.imageUri = mediaStoreAlloc("image/jpeg")
            takePicRes.launch(viewState.imageUri)
        }

        val captureVidRes =  registerForActivityResult(ActivityResultContracts.CaptureVideo()) {
            viewState.videoIcon = android.R.drawable.presence_video_busy
            view.videoButton.setImageResource(viewState.videoIcon)
        }
